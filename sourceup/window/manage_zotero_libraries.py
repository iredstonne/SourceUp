...
class ValidatedLineEdit(QWidget):
    def __init__(self, input_: QLineEdit, validator: QValidator | None = None, parent=None):
        ...
        self._error_indicator_idle_timer.timeout.connect(self._end_error_indicator_blink)
...
class ManageZoteroLibrariesWindow(QDialog):
    def __init__(self):
        ...
        self._form.addRow("Library ID", ValidatedLineEdit(
        ...
        self._private_access_checkbox.toggled.connect(self._on_private_access_toggled)
        self._form.addRow("Private Access", self._private_access_checkbox)
        self._api_key_input_label = QLabel("Private Key")
        self._api_key_input_label.setVisible(self._private_access_checkbox.isChecked())
        ...
        self._form.addRow(self._api_key_input_label, ValidatedLineEdit(
    ...
    def _select_list_idx(self, idx: QModelIndex):
        if idx.isValid():
            ...
            self._delete_all_button.setDisabled(False)

    def _clear_library_form(self):
        self._library_type_combo.setCurrentIndex(0)
        self._library_id_input.clear()
        self._library_id_input.setFocus()
    ...
    def _fill_library_form(self, library: ZoteroLibrary):
        ...
        self._library_id_input.setText(library.library_id)
        self._library_id_input.setFocus()
        self._private_key_input.setText(library.private_key)
    ...
    def _on_delete_button_clicked(self):
        idx = self._list_view.currentIndex()
        if idx.isValid():
            if QMessageBox.question(self, "Delete", "Delete this library?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No) == QMessageBox.StandardButton.Ok:
                self._model.remove_row(idx.row())
                self._clear_list_selection()
                self._clear_library_form()

    def _on_delete_all_button_clicked(self):
        if QMessageBox.question(self, "Delete all", "Delete all libraries?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No) == QMessageBox.StandardButton.Ok:
            self._model.reset_rows()
            self._clear_list_selection()
            self._clear_library_form()

    def _on_private_access_toggled(self):
        if self._private_access_checkbox.isChecked():
            self._api_key_input_label.show()
            self._private_key_input.show()
            self._private_key_help_label.show()
            return
        self._api_key_input_label.hide()
        self._private_key_input.hide()
        self._private_key_help_label.hide()
    ...
    def _on_save_button_clicked(self):
        ...
        if self._private_access_checkbox.isChecked() and not private_key:
